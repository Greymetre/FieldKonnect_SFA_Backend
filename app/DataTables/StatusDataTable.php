    ...
    public function dataTable($query)
    {
        return datatables()
            ...
            ->addColumn('action', function ($query) {
                  $btn = '';
                  $activebtn ='';
                  if(auth()->user()->can(['status_edit']))
                  {
                    $btn = $btn.'<a href="javascript:void(0)" class="btn btn-theme btn-just-icon btn-sm edit" id="'.encrypt($query->id).'" title="'.trans('panel.global.edit').' '.trans('panel.status.title_singular').'">
                          <i class="material-icons">edit</i>
                        </a>';
                  }
                  if(auth()->user()->can(['status_delete']))
                  {
                    $btn = $btn.' <a href="javascript:void(0)" class="btn btn-danger btn-just-icon btn-sm delete" value="'.$query->id.'" title="'.trans('panel.global.delete').' '.trans('panel.status.title_singular').'">
                                <i class="material-icons">clear</i>
                              </a>';
                  }
                  // if(auth()->user()->can(['status_active']))
                  // {
