...
public function attendanceSummaryDownload(Request $request)
{
    ...
    $data = $users->map(function ($user) use ($period, $last60Days, $holidayData, $start_date, $end_date) {
        ...
        foreach ($period as $date) {
            ...
            if ($attendance) {
                ...
                // Leave is shown according to the balance used. Comp-Off is
                // treated as Present, while Casual Leave has its own column.
                if (array_intersect($workingTypes, ['Full Day Leave', 'First Half Leave', 'Second Half Leave', 'Leave'])) {
                    $leave = Leave::where('user_id', $user->id)
                        ...
                        ->first();
                    $isCompOff = ($leave?->bal_type ?? '') === 'Comp-off Balance';
    ...
    // Keep approved/applied leave visible even if its attendance row has not
    // yet been generated by the leave-attendance command.
    $leave = Leave::where('user_id', $user->id)
        ->whereDate('from_date', '<=', $ymd)
        ->whereDate('to_date', '>=', $ymd)
        ->first();
