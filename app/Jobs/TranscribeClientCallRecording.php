...
class TranscribeClientCallRecording implements ShouldQueue
{
    ...
    {
        $call = ClientCallLog::find($this->clientCallLogId);
        if (! $call || ! $call->recording_url || $call->transcription_status === 'completed') return;

        $apiKey = config('services.sarvam.api_key');
        if (! $apiKey) throw new RuntimeException('SARVAM_API_KEY is not configured.');

        $call->update(['transcription_status' => 'processing', 'transcription_error' => null]);
        $audio = Http::withBasicAuth(
            config('services.client_calling.auth_id'),
            config('services.client_calling.auth_token')
        )->connectTimeout(10)->timeout(120)->get($call->recording_url)->throw();
        $contentType = $audio->header('Content-Type') ?: 'audio/mpeg';
        $extension = str_contains($contentType, 'wav') ? 'wav' : 'mp3';
        $fileName = 'client-call-'.$call->id.'.'.$extension;
        ...
        $call->update(['sarvam_job_id' => $jobId]);
    ...
    {
        ClientCallLog::whereKey($this->clientCallLogId)->update([
