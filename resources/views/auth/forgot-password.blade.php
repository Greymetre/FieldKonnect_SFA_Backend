...
<body>
  <main class="login-stage">
    ...
    <section class="panel">
      <div class="login">
        ...
        <p class="sub">Enter your email address and we will send password reset instructions if the account exists.</p>
        ...
        <form method="POST" action="{{ route('password.email') }}">
          ...
          <div class="field">
            ...
            <div class="ctrl">
              <input type="email" class="form-control @error('email') is-invalid @enderror" id="email" name="email" value="{{ old('email') }}" placeholder="you@company.com" required autofocus>
  ...
  <script>
    @if(session()->has('error'))
      console.error('Forgot password mail issue:', @json(session()->get('error')));
    @endif

    @if(session('status'))
      console.info('Forgot password mail flow completed:', @json(session('status')));
    @endif
