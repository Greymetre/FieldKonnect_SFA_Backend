<x-app-layout>
    ...
    <style>
        ...
        .customer-history-export { display:inline-flex;align-items:center;justify-content:center;gap:8px;height:43px;padding:0 17px;border:1px solid rgba(85,126,218,.38);border-radius:11px;background:rgba(7,20,49,.62);color:#c7d5f5;font-size:13px;font-weight:800;text-decoration:none; }
        .customer-history-export:hover { border-color:rgba(34,211,238,.5);color:#2dd4ee; }
        .customer-history-export .material-icons { font-size:20px; }
        .customer-history-filters { display:grid;grid-template-columns:minmax(220px,1.35fr) repeat(3,minmax(155px,.75fr)) auto auto;align-items:end;gap:12px;margin-bottom:14px;padding:16px;border:1px solid rgba(85,126,218,.27);border-radius:14px;background:rgba(7,20,49,.54); }
        .customer-history-filter label { display:block;margin-bottom:7px;color:#8395c4;font-size:11px;font-weight:800;letter-spacing:.1em;text-transform:uppercase; }
        .customer-history-filter input,.customer-history-filter select { width:100%;height:42px;padding:0 13px;border:1px solid rgba(85,126,218,.32);border-radius:10px;outline:0;background:#071938;color:#d5e0fa;font-size:13px; }
        ...
        .customer-history-apply,.customer-history-reset { display:inline-flex;align-items:center;justify-content:center;height:42px;padding:0 17px;border-radius:10px;font-size:13px;font-weight:800;text-decoration:none;white-space:nowrap; }
        ...
        @media (max-width:1200px) { .customer-history-filters { grid-template-columns:repeat(2,minmax(0,1fr)); } }
        @media (max-width:640px) { .customer-history-heading { align-items:flex-start; } .customer-history-heading h1 { font-size:22px; } .customer-history-export span:not(.material-icons) { display:none; } .customer-history-export { width:43px;padding:0; } .customer-history-filters { grid-template-columns:1fr; } }
    </style>
    <div class="customer-history">
        ...
        <form class="customer-history-filters" method="GET" action="{{ route('customer-call-history.index') }}">
            <div class="customer-history-filter">
                ...
                <input id="historySearch" type="search" name="search" value="{{ request('search') }}" placeholder="Firm, contact, mobile or notes">
            </div>
            @role('superadmin')
                <div class="customer-history-filter">
                    <label for="historyAgent">Agent</label>
                    <select id="historyAgent" name="agent_id">
                        <option value="">All agents</option>
                        @foreach($agents as $agent)<option value="{{ $agent->id }}" @selected((string) request('agent_id') === (string) $agent->id)>{{ $agent->name }}</option>@endforeach
                    </select>
                </div>
            @endrole
            <div class="customer-history-filter">
                ...
                <select id="historyCallStatus" name="call_status">
                    ...
                    @foreach(['completed' => 'Completed', 'initiated' => 'Initiated', 'queued' => 'Queued', 'ringing' => 'Ringing', 'failed' => 'Failed', 'busy' => 'Busy', 'no-answer' => 'No Answer'] as $value => $label)
                        <option value="{{ $value }}" @selected(request('call_status') === $value)>{{ $label }}</option>
                    @endforeach
                </select>
            </div>
            <div class="customer-history-filter">
                ...
                <select id="historyAgentStatus" name="feedback_status_id">
                    ...
                    @foreach($feedbackStatuses as $feedbackStatus)
                        <option value="{{ $feedbackStatus->id }}" @selected((string) request('feedback_status_id') === (string) $feedbackStatus->id)>{{ $feedbackStatus->display_name ?: $feedbackStatus->status_name }}</option>
                    @endforeach
                </select>
            </div>
